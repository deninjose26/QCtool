...
const QCReviewQueue: React.FC = () => {
    ...
    const [recordNameFilter, setRecordNameFilter] = useState('all');
    ...
    const fetchHistory = async () => {
        try {
            ...
            const pendingData = data.filter((t: any) => t.qc_batch_status === 'Completed');
            const mappedData = pendingData.map((t: any) => ({ ...t, id: t.qc_allocation_id }));
            setHistory(mappedData);
            setFilteredHistory(mappedData);
        } catch (error) {
            toast({
                title: 'Error',
                description: 'Could not load QC review queue',
                variant: 'destructive'
            });
        } finally {
            setIsLoading(false);
    ...
    useEffect(() => {
        ...
        if (recordNameFilter !== 'all') result = result.filter(t => t.record_name === recordNameFilter);
    ...
    }, [searchTerm, history, projectFilter, sourceFilter, locationFilter, ownerFilter, typeFilter, userFilter, recordNameFilter, dateRange]);
    ...
    const records = Array.from(new Set(history.map(t => t.record_name))).sort();
    ...
    const handleVerifyBatch = async (task: QCSupHistoryTask) => {
        if (!window.confirm(`Are you sure you want to verify batch ${task.batch_id}?`)) return;

        try {
            const res = await apiFetch(`${API_BASE_URL}/qc-sup/verify-batch/${task.batch_uid}`, {
                method: 'POST'
            });

            if (!res.ok) throw new Error('Failed to verify batch');
            ...
            toast({
                title: 'Error',
                description: 'Could not verify batch',
                variant: 'destructive'
            });
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, item: QCSupHistoryTask) => (
                <div className="flex items-center gap-2">
                    {item.qc_batch_status === 'Completed' && (
                        <>
                            <Button
                                variant="outline"
                                size="icon"
                                onClick={() => navigate(`/qc-review/${item.batch_uid}`)}
                                title="Review"
                                className="h-7 w-7 border-purple-200 text-purple-700 hover:bg-purple-50 hover:text-purple-800"
                            >
                            ...
                            <Button
                                variant="default"
                                size="sm"
                                onClick={() => handleVerifyBatch(item)}
                                className="h-7 px-3 text-[10px] font-bold bg-indigo-600 hover:bg-indigo-700"
                            >
                                VERIFY
                            </Button>
                        </>
        ...
        <div className="space-y-6 animate-fade-in pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <PageHeader
                    ...
                    description="Batches awaiting supervisor verification"
                />
                ...
                <div className="flex flex-col gap-2 p-3">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 flex-none">
                            ...
                            {(searchTerm || projectFilter !== 'all' || sourceFilter !== 'all' || userFilter !== 'all' || locationFilter !== 'all' || ownerFilter !== 'all' || typeFilter !== 'all' || recordNameFilter !== 'all' || dateRange) && (
                                <Button
                                    variant="ghost"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
                        ...
                        <div className="relative group/filter">
                            ...
                            <Select value={userFilter} onValueChange={setUserFilter}>
                                ...
                                <SelectContent>
                                    <SelectItem value="all">Every QC User</SelectItem>
                        ...
                        <div className="relative group/filter">
                            ...
                            <Select value={ownerFilter} onValueChange={setOwnerFilter}>
                                ...
                                <SelectContent>
                                    <SelectItem value="all">Every Record Owner</SelectItem>
                        ...
                        <div className="relative group/filter">
                            ...
                            <Select value={typeFilter} onValueChange={setTypeFilter}>
                                ...
                                <SelectContent>
                                    <SelectItem value="all">Every Record Type</SelectItem>
                        ...
                        <div className="relative group/filter">
                            ...
                            <Select value={recordNameFilter} onValueChange={setRecordNameFilter}>
                                <SelectTrigger className="h-9 pl-9 bg-white border-slate-200/50 rounded-lg text-[11px] font-semibold focus:ring-4 focus:ring-indigo-500/5 hover:border-indigo-200 transition-all">
                                    <SelectValue placeholder="Name" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="all">Every Book Name</SelectItem>
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
                    <p className="text-slate-500">No pending reviews found in the queue.</p>
                </div>
            ) : (
