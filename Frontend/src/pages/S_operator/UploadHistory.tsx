...
const OperatorUploadHistory: React.FC = () => {
    ...
    const columns = [
        {
            ...
            header: 'Batch ID',
            sortable: true,
            render: (val: string, item: OperatorBatch) => (
                <code className={cn(
        ...
        },
        {
            ...
            render: (_: any, item: OperatorBatch) => {
                const percentage = Math.round((item.completed_count / (item.target_count || 1)) * 100) || 0;
                return (
                    <div className="flex flex-col gap-1 min-w-[140px]">
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-tighter">
                            ...
                            <span>{item.completed_count} / {item.target_count}</span>
                        </div>
        ...
        },
        {
            ...
            render: (value: string) => (
                <Badge className={cn(
                    ...
                    value === 'Complete' ? "bg-emerald-100 text-emerald-700 border-emerald-200" :
                        value === 'Partial' ? "bg-blue-100 text-blue-700 border-blue-200" :
    ...
    return (
        <div className="space-y-4 animate-fade-in">
                ...
                <div className="flex flex-col gap-2 p-3">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="relative flex-1 min-w-[320px] group">
                            ...
                            <Input
                                placeholder="Search history by Batch ID, Book, Project..."
                                value={searchTerm}
            ...
            <div className="flex items-center gap-2 p-4 bg-muted/30 border rounded-lg text-muted-foreground text-xs mt-4">
                <Info className="h-4 w-4" />
                <p>Only batches that have been 100% uploaded and verified on S3 are shown here. For active uploads, please check the Active Uploads page.</p>
            </div>
        </div>
    );
};
