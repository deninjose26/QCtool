...
const AdminQCHistory: React.FC = () => {
    const { apiFetch } = useAuth();
    const navigate = useNavigate();
    const [history, setHistory] = useState<AdminQCHistoryTask[]>([]);
    const [filteredHistory, setFilteredHistory] = useState<AdminQCHistoryTask[]>([]);
    ...
    const fetchHistory = async () => {
        try {
            setIsLoading(true);
            const res = await apiFetch(`${API_BASE_URL}/admin/qc-history`);
    ...
    const handleDownloadBatch = async (task: AdminQCHistoryTask) => {
        try {
    ...
    const columns = [
        ...
        },
        {
            key: 'actions',
            header: 'Actions',
            render: (_: any, item: AdminQCHistoryTask) => (
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
                        size="icon"
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <PageHeader
                    title="System-Wide QC History"
                    description="Full audit trail of all verified quality control batches across the organization"
                />
            ...
            {isLoading ? (
                <div className="flex flex-col justify-center items-center py-32 gap-4">
                    ...
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-[0.2em] animate-pulse">Syncing Master QC Records...</p>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-20 text-center animate-in fade-in zoom-in duration-300">
                    ...
                    <p className="text-slate-500 font-medium">No system QC results match your specialized criteria.</p>
                </div>
            ) : (
            ...
            <div className="p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl text-slate-500 text-[11px] mt-4 flex items-center gap-3 backdrop-blur-sm">
                ...
                <div>
                    ...
                    <p>You are accessing the global QC audit trail. Every verification event and rejection remark is being tracked and logged under your session ID: <code className="bg-white px-1.5 py-0.5 rounded border border-indigo-200 text-indigo-700 font-bold">{token?.slice(-8)}</code></p>
                </div>
            </div>
        </div>
    );
