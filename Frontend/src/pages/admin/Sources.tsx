...
const Sources: React.FC = () => {
  ...
  const fetchSources = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // We need to map project names here as well, so we use projects state
  ...
  // When projects are loaded or updated, refresh sources to get project names
  useEffect(() => {
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingSource ? 'Update the source details.' : 'Add a new document source.'}
            </DialogDescription>
          </DialogHeader>
